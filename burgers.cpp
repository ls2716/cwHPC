...
Burgers::Burgers(Model& m)
{
    ...
    // pre - Calculating coefficients for integration
    ...
    */
	...
	//Precalculated smaller sizes for faster calculations
	smy_Nx=my_Nx-1;
	smy_Ny=my_Ny-1;
	...
	*/
	...
	//Initializing arrays to hold the values
	my_size = my_Ny*my_Nx;
	...
	//Assigning correct pointer for the initial state 
	vgrid_out = vgrid_e;
	ugrid_out = ugrid_e;
	
	if (my_rank==0)
	{
        //Defining new global grid and its parameters
		int full_Nx=Nx+2;
		int full_Ny=Ny+2;
...
//Releasing the memory in the desctructor
Burgers::~Burgers()
{
...
void Burgers::Initialize()
{
	...
	for (int j=0; j<my_Ny; j++)
	{
		...
		//Updating own right and left boundaries have to be sent at the beginning of each time step
	...
	//Updating own top and bottom boundaries have to be sent at the beginning of each time step
	for (int i=0; i<my_Nx; i++)
	{
...
// Cross sending values in the boundaries between the processes
void Burgers::BoundaryUpdate()
{
    Bcounter=0;
    if (downB) //Sending and receiving down
    {
    ...
    if (upB) //Sending and receiving up
    {
    ...
    if (leftB) //Sending and receiving left
    {
    ...
    if (rightB) //Sending and receiving right
    {
...
/*Calculating energy
 * Each process calculates energy of its own subgrid
 * and then send this value to the process 0
 */
void Burgers::Energy()
{
...
void Burgers::Integrate()
{
	...
	// Iterating from time step equal 0 make the next time step
	nt=0;
	while (nt<Nt)
	{
        NextStep(); 
...
//Function to call at the finish of the integration to assemble the grid, get energy and print it to a file "grid.txt"
void Burgers::WrapUp()
{
...
void Burgers::WriteToFile()
{
	...
	/*Writing - first line with Nx and Ny
	* Then u matrix as naturally exists
	* Then again line with Nx and Ny
	* The v matrix as it normally exists
	*/
	cout.precision(8); //Setting precision
...
void Burgers::Assemble()
{
	if (my_rank==0)
	{
		...
		//Parameters of processes being received
		int m_Nx;
		int m_Ny;
		int m_i0;
		int m_j0;
